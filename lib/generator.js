...
Generator.prototype.generateAPI = function (apiDiscoveryUrl, callback) {
  ...
  function _generate (err, resp) {
    ...
    // Functions
    var exportFilename2 = "/Users/legnitto/src/pocketship/server/rewrite/GooglePathToPocketshipMapper.js";
    var templateContents2 = fs.readFileSync('./templates/functions.js', { encoding: 'utf8' });
    var contents2;
    async.waterfall([
      function (cb) {
        async.parallel(tasks, cb);
      ...
      function (results, cb) {
        var result = swig.render(templateContents2, { locals: resp })
          .replace(new RegExp(googleApisBaseUrl.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&'), 'g'), overriddenBaseUrl);
        contents2 = replacer(beautify(result, BEAUTIFY_OPTIONS));

        //console.log(contents);

        mkdirp(path.dirname(exportFilename2), cb);
      },
      function (cb) {
        fs.writeFile(exportFilename2, contents2, { encoding: 'utf8' }, cb);
      }
    ], function (err) {
      if (err) {
        return handleError(err, callback);
      }
      return callback(null, exportFilename2);
    });
  }
